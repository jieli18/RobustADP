...
class _StripPlotter(_CategoricalScatterPlotter):
    ...
    def __init__(self, x, y, hue, data, order, hue_order,
        ...
        if color is not None:
            self.colors = color
            light_vals = [colorsys.rgb_to_hls(*c)[1] for c in self.colors]
            lum = min(light_vals) * .6
            self.gray = mpl.colors.rgb2hex((lum, lum, lum))
...
def RainCloud(x=None, y=None, hue=None, data=None,
              ...
              color=None, palette=None, bw=.2, linewidth=1, cut=0.,
    ...
    if color is None and palette is None:
        palette = "Set2"
    assert not (color is not None and palette is not None), "color and palette cannot have content at the same time."
    ...
    kwcloud = dict()
    kwbox = dict(saturation=1, whiskerprops={'linewidth': 1, "zorder": 10})
    ...
    half_violinplot(x=x, y=y, hue=hue, data=data,
                    ...
                    inner=None, color=color, palette=palette, bw=bw, linewidth=linewidth,
    ...
    sns.boxplot(x=x, y=y, hue=hue, data=data, orient=orient, width=width_box,
                order=order, hue_order=hue_order,
                color=boxcolor, showcaps=True, boxprops=boxprops, linewidth=linewidth,
                palette=palette, dodge=dodge, whis=2.5, ax=ax, **kwbox)
    ...
    ax = stripplot(x=x, y=y, hue=hue, data=data, orient=orient,
                   order=order, hue_order=hue_order, color=color, palette=palette,
    ...
    font_label = {'family': font_name,
                  'size': 12,
                  'weight': 'normal',
                  }
    ax.set_xlabel(xlabel=x, fontdict=font_label)
    ax.set_ylabel(ylabel="attenuation", fontdict={'family': 'Times New Roman', 'size': 12, 'weight': 'normal'})
