...
class _GymVibration(gym.Env):
    def __init__(self, **kwargs):
        ...
        self.k_0 = 2.0
        self.m = 3.0
        self.bound_delta_b = 0.5 * abs(self.b_0)
        self.bound_delta_k = 0.5 * abs(self.k_0)
        self.scale_delta_b = 0.  # [-1, 1]
        self.scale_delta_k = 0.  # [-1, 1]
        self.b = self.b_0 + self.scale_delta_b * self.bound_delta_b
        self.k = self.k_0 + self.scale_delta_k * self.bound_delta_k
        self.A = np.array([[0.0, 1.0],
                           [-self.k / self.m, -self.b / self.m]], dtype=np.float32)
        self.B = np.array([0., 1 / self.m], dtype=np.float32).reshape((2, 1))
        self.D = np.array([0., 1 / self.m], dtype=np.float32).reshape((2, 1))
        ...
        self.observation_space = spaces.Box(low=np.array([-self.position_threshold, -self.velocity_threshold]),
                                            high=np.array([self.position_threshold, self.velocity_threshold]),
                                            shape=(2,)
                                            )
    ...
    def stepPhysics(self, action, adv_action):
        ...
        f_dist = adv_action[0]  # noise

        position_dot = velocity
        velocity_dot = - self.k / m * position - self.b / m * velocity + 1 / m * (force + f_dist)

        next_position = position_dot * tau + position
        next_velocity = velocity_dot * tau + velocity
    ...
    def step(self, inputs):
        ...
        next_position, next_velocity = self.state
        done = next_position < -self.position_threshold or next_position > self.position_threshold \
            or next_velocity < -self.velocity_threshold or next_velocity > self.velocity_threshold
        done = bool(done)
        ...
        if not done:
            reward = self.Q[0][0] * position ** 2 + self.Q[1][1] * velocity ** 2 \
                     + self.R[0][0] * force ** 2 - self.gamma_atte ** 2 * (f_dist ** 2)
...
Any further steps are undefined behavior.
        ...
        reward_positive = self.Q[0][0] * position ** 2 + self.Q[1][1] * velocity ** 2 + self.R[0][0] * force ** 2
        reward_negative = f_dist ** 2
...
def env_creator(**kwargs):
    return TimeLimit(_GymVibration(**kwargs), _GymVibration(**kwargs).max_episode_steps)  # original = 200
