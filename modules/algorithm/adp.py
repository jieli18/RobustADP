...
#  Adaptive Dynamic Programming Algorithm (ADP)
...
class ADP:
    ...
    def __init__(self, **kwargs):
        ...
        self.scale_lyapunov = kwargs.get('scale_lyapunov', False)
        self.norm_matrix = kwargs.get('norm_matrix', [1.0 for i in range(kwargs['obsv_dim'])])
        ...
        if self.scale_lyapunov:
            self.scale_state_in_lyapunov = torch.from_numpy(np.array(self.norm_matrix, dtype=np.float32))
        else:
            self.scale_state_in_lyapunov = torch.from_numpy(np.array([1.0 for i in range(kwargs['obsv_dim'])], dtype=np.float32))
        # ------------------------------------
    ...
    def compute_loss_value(self, data):
        ...
        clip_stability, derivative_lyapunov = self._derivative_lyapunov(state, delta_value)  # torch.Size([batch, 1])
    ...
    def _estimated_hamiltonian(self, state, delta_value):
        ...
        if batch_size > 1:
            ...
            hamiltonian = Q_x + dv_fx + 1/4 * torch.bmm(torch.bmm(delta_value[:, np.newaxis, :], D_d),
            ...
            hamiltonian = Q_x + dv_fx + 1/4 * torch.mm(torch.mm(delta_value, D_d), delta_value.t())
    ...
    def _derivative_lyapunov(self, state, delta_value):
        ...
        if batch_size > 1:
            ...
            derivative_lyapunov = torch.bmm(torch.mul(state, self.scale_state_in_lyapunov)[:, np.newaxis, :],
                                            torch.mul(delta_state, self.scale_state_in_lyapunov)[:, :, np.newaxis]).squeeze(-1)  # [64, 1]
        else:
            D_d = 1 / gamma_atte ** 2 * torch.mm(self.env_model.k_x(state, 1), self.env_model.k_x(state, 1).t()) \
                  - torch.mm(torch.mm(self.env_model.g_x(state, 1), torch.inverse(R)), self.env_model.g_x(state, 1).t())
            delta_state = self.env_model.f_x(state, 1) + 1/2 * torch.mm(D_d, delta_value.t())  # [2, 1]
            derivative_lyapunov = torch.mm(torch.mul(state, self.scale_state_in_lyapunov),
                                           torch.mul(delta_state.t(), self.scale_state_in_lyapunov).t())  # [1, 1]
...
if __name__ == '__main__':
    batch_size = 7
    derivative_lyapunov = torch.tensor([[-2], [-1], [-0.5], [0], [0.5], [1], [2]])
    clip_stability = torch.where(derivative_lyapunov[:, 0] < 0, torch.zeros(batch_size), torch.ones(batch_size))[:, np.newaxis]
    print(clip_stability)
    state = torch.tensor([[13, 13, 13, 13],
                          [19, 19, 19, 19]])
    norm_matrix = torch.tensor([2, 3, 5, 7])
    print(torch.mul(state, norm_matrix))
