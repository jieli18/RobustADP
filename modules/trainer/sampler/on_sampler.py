...
class OnSampler():
    def __init__(self, **kwargs):
        ...
        self.probing_noise = kwargs['probing_noise']
        ...
        if 'adversary_dim' in kwargs.keys():  # TODO?
    ...
    def sample(self):
        ...
        for _ in range(self.sample_batch_size):
            ...
            if self.probing_noise:
                probing_noise = self.env.exploration_noise(self.total_sample_number * self.env.tau)
                action = action + probing_noise
            else:
                probing_noise = None
            ...
            if info['TimeLimit.truncated']:
                self.done = False
            data = [self.obs.copy(), action[:self.act_dim], reward, next_obs.copy(), self.done, logp, info['TimeLimit.truncated']]
            if self.is_constrained:
                constraint = info['constraint']
            else:
                constraint = None
            if self.is_adversary:
                sth_about_adversary = action[self.act_dim:]
            else:
                sth_about_adversary = None
            data.append(constraint)
            data.append(sth_about_adversary)
            data.append(probing_noise)
    ...
    def samples_conversion(self, samples):
        ...
        if self.probing_noise:
            tensor_dict['probing'] = torch.zeros(self.sample_batch_size, self.act_dim + self.advers_dim)
        else:
            tensor_dict['probing'] = None
        idx = 0
        for sample in samples:
            ...
            if self.probing_noise:
                probing = sample[-1]
                tensor_dict['probing'][idx] = torch.from_numpy(probing)
            idx += 1
