...
class OffSerialTrainer():
    ...
    def step(self):
        ...
        # insert
        replay_samples['obs'] = torch.vstack(
            (replay_samples['obs'], torch.tensor(sampler_samples[0][0], dtype=torch.float32).unsqueeze(dim=0)))
        ...
        if self.iteration % self.log_save_interval == 0:
            ...
            self.state_history[int(self.iteration - 1 / self.log_save_interval)] = sampler_samples[0][0]
            self.action_history[int(self.iteration - 1 / self.log_save_interval)] \
                = np.hstack((sampler_samples[0][1], sampler_samples[0][-1]))
        # evaluate
    ...
    def train(self):
        while self.iteration < self.max_iteration:
            self.step()
        print(f'time = {(time.time() - self.start_time):.3f} s')
