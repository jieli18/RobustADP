...
def build_parser():
    ...
    if mode != "train":
        ...
        test_log_dir = r'E:\GitHub\RobustADP\results\RADP\250112-181451'
        network_number = '3000'
        dist_func_type = 'sine'
        plot_data = {'sine': r'\test_sine_3000-0112-204610',
                     'white': r'\test_white_3000-0112-205109'}
        scale = 1.0  # 0.1
        ...
        if mode == "test":
            ...
            data_dir = test_log_dir
            data_dir_list = [test_log_dir + r'\test_sine_3000-0514-201101',
                             test_log_dir + r'\test_white_3000-0514-202202']
    ...
    # Initial state
    parser.add_argument('--fixed_initial_state', type=list, default=[2.0, 2.0, -1.0], help='for env_data')
    ...
    if mode == 'train':
        ...
        if mode == 'test':
            ...
            simulator.networks.load_state_dict(
                torch.load(r'E:\GitHub\RobustADP\results\ADP\250112-164705\apprfunc\apprfunc_10000.pkl'))
            for i in range(len(scale_list)):
                print(f'simulation for game: {i + 1} / {len(scale_list)}')
            ...
            # w/o
            simulator.networks.load_state_dict(
                torch.load(r'E:\GitHub\RobustADP\results\RADPWOUCC\250112-171729\apprfunc\apprfunc_3000.pkl'))
            for i in range(len(scale_list)):
                print(f'simulation for w/o: {i + 1} / {len(scale_list)}')
                simulator.env.env.scale_delta_p = scale_list[i]
                ...
                color_list.append(color_dict['w/o'])
            ...
            save_data(data=np.stack(attenuation_game, axis=1), row=num_data, column=num_line,
                      save_file=args['save_folder'], xls_name='/eg3_attenuation_game_' + args['dist_func_type'])
            ...
            workbook = xlrd.open_workbook(data_dir + '/eg3_attenuation_game_' + args['dist_func_type'] + '.xls')
            ...
            attenuation_wo_ucc_sine = []
            attenuation_radp_sine = []
            data_dir = data_dir_list[0]
            num_data = num_data_list[0] - 1

            workbook = xlrd.open_workbook(data_dir + '/eg3_attenuation_game_' + dist_func_type + '.xls')
            sheet = workbook.sheets()[0]
            for j in range(0, num_line):
            ...
            workbook = xlrd.open_workbook(data_dir + '/eg3_attenuation_wo_ucc_' + dist_func_type + '.xls')
            sheet = workbook.sheets()[0]
            for j in range(0, num_line):
                l2_gain_wo_ucc_sine = np.ones((1, 1))
                l2_gain_wo_ucc_sine[0, 0] = sheet.cell_value(int(ratios[0] * num_data), j)
                attenuation_wo_ucc_sine.append(l2_gain_wo_ucc_sine[:, 0])
                color_list.append(color_dict['w/o'])

            workbook = xlrd.open_workbook(data_dir + '/eg3_attenuation_radp_' + dist_func_type + '.xls')
            sheet = workbook.sheets()[0]
            for j in range(0, num_line):
            ...
            sheet = workbook.sheets()[0]
            for j in range(0, num_line):
                l2_gain_wo_ucc_white = np.ones((1, 1))
                l2_gain_wo_ucc_white[0, 0] = sheet.cell_value(int(ratios[0] * num_data), j)
                attenuation_wo_ucc_white.append(l2_gain_wo_ucc_white[:, 0])
                color_list.append(color_dict['w/o'])
            ...
            present_raincloud(
                datas=[np.stack(attenuation_game_sine + attenuation_wo_ucc_sine + attenuation_radp_sine, axis=1),
                       np.stack(attenuation_game_white + attenuation_wo_ucc_white + attenuation_radp_white, axis=1)],
                column=num_line, methods=label_list, titles=['(a) Sinusoidal ', '(b) White noise'],
                ylim=[(2.003, 2.157), (1.87, 2.03)], point_size=2.0,
                pad=0.0, wspace=0.28,
                color=[cloud_dict['game_cloud'], cloud_dict['wo_ucc_cloud'], cloud_dict['radp_cloud']],
                figure_name=args['save_folder'] + '/eg3_raincloud_present',
                display=True)

        else:
            raise ValueError("Error mode!")

        # my_plot(data=np.stack(x1_game + x1_wo_ucc + x1_radp, axis=1), time=time,
        #         figure_size_scalar=1,
        ...
        #         figure_type='png',
        #         display=False)
        # my_plot(data=np.stack(x2_game + x2_wo_ucc + x2_radp, axis=1), time=time,
        #         figure_size_scalar=1,
        #         color_list=color_list, label_list=label_list,
        #         loc_legend='upper right', ncol=1,
        #         xlim=(0, time[-1, 0]), ylim=None,
        ...
        #         figure_type='png',
        #         display=False)
        # my_plot(data=np.stack(x3_game + x3_wo_ucc + x3_radp, axis=1), time=time,
        #         figure_size_scalar=1,
        #         color_list=color_list, label_list=label_list,
        #         loc_legend='upper right', ncol=1,
        #         xlim=(0, time[-1, 0]), ylim=None,
        ...
        #         display=False)
        #
        # my_plot(data=np.stack(control_game + control_wo_ucc + control_radp, axis=1), time=time,
        #         figure_size_scalar=1,
        #         color_list=color_list, label_list=label_list,
        #         loc_legend='upper right', ncol=1,
        #         xlim=(0, time[-1, 0]), ylim=None,
        ...
        single_raincloud(data=np.stack(attenuation_game + attenuation_wo_ucc + attenuation_radp, axis=1),
        ...
        double_raincloud(data=np.stack(attenuation_game + attenuation_wo_ucc + attenuation_radp, axis=1),
                         row=num_data, column=num_line, methods=label_list,
                         ylim=ylim, ratio=ratio, point_size=2.0,
                         color=[cloud_dict['game_cloud'], cloud_dict['wo_ucc_cloud'], cloud_dict['radp_cloud']], palette=None,
                         figure_name=args['save_folder'] + '/eg3_double_raincloud_' + args['dist_func_type'])
        my_plot(data=np.stack(attenuation_game + attenuation_wo_ucc + attenuation_radp, axis=1), time=time,
                figure_size_scalar=1,
                color_list=color_list, label_list=label_list,
